...
{
    for(size_t i=0; i<listePage.size();++i)
    {
        delete(listePage[i]);
    }
    delete ui;
...
void MainWindow::on_actionDebut_triggered()
{
    QString balise = "<a href=\"#"+ui->textEdit->textCursor().selectedText()+"\">"+ui->textEdit->textCursor().selectedText()+"</a>";
    listeBalise.push_back(ui->textEdit->textCursor().selectedText());
    lastBalise = ui->textEdit->textCursor().selectedText();
    ui->textEdit->insertHtml(balise);
}

void MainWindow::on_actionFIn_triggered()
{
    QString finBalise = "<p id=\""+lastBalise+"\">"+ui->textEdit->textCursor().selectedText()+"</p>";
    ui->textEdit->insertHtml(finBalise);
}


void MainWindow::on_ajouterPage_clicked()
{
    nbPages++;
    ui->comboBox->addItem("Page "+QString::number(nbPages));
    QString *document = new QString(ui->textEdit_2->toPlainText());
    listePage.push_back(document);
    ui->textEdit_2->clear();
    ui->textEdit->clear();
    ui->comboBox->setCurrentIndex(nbPages-1);
    PageActuel = nbPages-1;
}


void MainWindow::on_comboBox_activated(int index)
{
    *listePage[PageActuel]= ui->textEdit_2->toPlainText();
    PageActuel=index;
    ui->textEdit_2->setPlainText(*listePage[index]);
    ui->textEdit->setHtml(ui->textEdit_2->toPlainText());
...
{
    QString Titre = "<h1>"+ui->textEdit->textCursor().selectedText()+"</h1>";
    ui->textEdit->insertHtml(Titre);
}
...
{
    PageActuel++;
    ui->comboBox->setCurrentIndex(PageActuel);
    ui->textEdit_2->setPlainText(*listePage[PageActuel]);
    ui->textEdit->setHtml(ui->textEdit_2->toPlainText());

}


void MainWindow::on_Precedant_clicked()
{
    PageActuel--;
    ui->comboBox->setCurrentIndex(PageActuel);
    ui->textEdit_2->setPlainText(*listePage[PageActuel]);
